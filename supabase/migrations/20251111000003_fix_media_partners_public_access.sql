-- Migration pour permettre l'accès public aux partenaires médias actifs
-- Et insérer les partenaires Prisma Media

-- 1. Modifier le statut pour accepter 'active'
ALTER TABLE media_partners DROP CONSTRAINT IF EXISTS media_partners_status_check;
ALTER TABLE media_partners ADD CONSTRAINT media_partners_status_check 
  CHECK (status IN ('pending', 'approved', 'rejected', 'suspended', 'active'));

-- 2. Ajouter une policy pour voir les partenaires actifs publics (sans user_id)
DROP POLICY IF EXISTS "Anyone can view active public media partners" ON media_partners;
CREATE POLICY "Anyone can view active public media partners"
  ON media_partners FOR SELECT
  TO authenticated
  USING (status = 'active' AND user_id IS NULL);

-- 3. Insérer les partenaires médias Prisma Media
...
-- 4. Créer un index unique sur le nom si pas déjà existant
CREATE UNIQUE INDEX IF NOT EXISTS idx_media_partners_name ON media_partners(name);

-- 5. Vérification : afficher tous les partenaires actifs
SELECT name, category, audience_size, monthly_visitors, status 
FROM media_partners 
WHERE status = 'active'
