...
export const QuizModuleRenderer: React.FC<QuizModuleRendererProps> = ({
  ...
  const renderModule = (m: Module) => {
      ...
      return (
        ...
        >
          <div style={{ display: 'flex', justifyContent, width: '100%' }}>
            <div
              style={{
                // Mobile: pleine largeur mais hauteur contrôlée (~40% de l'écran)
                width: '100%',
                maxWidth: device === 'mobile' ? '100%' : maxContentWidth,
                borderRadius,
                overflow: 'hidden',
                ...
                height: device === 'mobile' ? '40vh' : containerHeight,
                maxHeight: device === 'mobile' ? '40vh' : undefined,
    ...
    if (m.type === 'BlocBouton') {
      ...
      // FORMEDITOR: Ne pas afficher les boutons "Participer" ou "Rejouer"
      const label = (buttonModule.label || '').trim().toLowerCase();
      if (label === 'participer' || label === 'rejouer') {
        return null; // Masquer ces boutons
      }
    ...
    if (m.type === 'BlocPiedDePage') {
      ...
      const linkColor = footerModule.linkColor ?? '#E0004D';
