...
const { syncAllStates } = useCampaignStateSync();
  ...
  useEffect(() => {
    ...
    if (!campaignId || !isValidUuid(campaignId)) {
      if (!modeParam) {
        console.log('🎯 [FormEditor] No campaign ID and no mode → showing mode selection');
        setShowModeSelection(true);
      } else {
        console.log('🎯 [FormEditor] Mode already selected:', modeParam);
  ...
  const handleModeSelect = (selectedMode: 'fullscreen' | 'article') => {
    console.log('🎯 [FormEditor] Mode selected:', selectedMode);
    setShowModeSelection(false);
...
useEffect(() => {
  ...
  const handleSaveCampaignName = useCallback(async () => {
    ...
    const payload: any = isUuid(currentId) ? { id: currentId, name } : { name, type: 'form' };
    ...
    if (updated) {
      ...
      try { localStorage.setItem(`campaign:name:prompted:${(updated as any)?.id || currentId || 'new:form'}`, '1'); } catch {}
