...
const buildLaunchButtonStyles = (
  ...
  formStyleOverrides: Record<string, any>,
  formConfig: {
  ...
  const resolvedBorderRadius =
    formStyleOverrides.borderRadius ||
    moduleStyles.borderRadius ||
    (typeof formConfig.borderRadius === 'number'
      ? `${formConfig.borderRadius}px`
      : formConfig.borderRadius) ||
    '12px';

  return {
    background:
      moduleStyles.background ||
      formStyleOverrides.buttonBackgroundColor ||
      formConfig.buttonBackgroundColor ||
      LAUNCH_BUTTON_FALLBACK_GRADIENT,
    color:
      moduleStyles.color ||
      formStyleOverrides.buttonTextColor ||
      formConfig.buttonTextColor ||
      LAUNCH_BUTTON_DEFAULT_TEXT_COLOR,
    padding:
      moduleStyles.padding ||
      formStyleOverrides.buttonPadding ||
      LAUNCH_BUTTON_DEFAULT_PADDING,
    borderRadius: resolvedBorderRadius,
    boxShadow:
      moduleStyles.boxShadow ||
      formStyleOverrides.buttonBoxShadow ||
      LAUNCH_BUTTON_DEFAULT_SHADOW,
    display: formStyleOverrides.buttonDisplay || 'inline-flex',
    alignItems: formStyleOverrides.buttonAlignItems || 'center',
    justifyContent: formStyleOverrides.buttonJustifyContent || 'center',
    minWidth: formStyleOverrides.buttonMinWidth,
    minHeight: formStyleOverrides.buttonMinHeight,
    width: formStyleOverrides.buttonWidth,
    height: formStyleOverrides.buttonHeight,
    textTransform: formStyleOverrides.buttonTextTransform,
    fontWeight: formStyleOverrides.buttonFontWeight || 600
...
const FormEditorLayout: React.FC<FormEditorLayoutProps> = ({ mode = 'campaign', hiddenTabs }) => {
  ...
  const getTemplateBaseWidths = useCallback(() => {
    const width = 450; // Fixed width for forms
    const mobileWidth = Math.round(width * 0.85);
    return { desktop: `${width}px`, mobile: `${mobileWidth}px` };
  }, []);

  // Réinitialiser les backgrounds temporaires UNIQUEMENT au premier montage
  useEffect(() => {
    try {
  ...
  const initialTemplateWidths = useMemo(() => getTemplateBaseWidths(), [getTemplateBaseWidths]);
