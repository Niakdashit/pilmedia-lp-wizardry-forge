    ...
    <div className="max-w-md mx-auto p-4">
      <div className="flex flex-wrap justify-center gap-8">
        {(diceValues.length ? diceValues : [1, 1]).map((value, index) => (
          <motion.div
            key={index}
            className="w-24 h-24 bg-white rounded-2xl shadow-xl flex items-center justify-center text-4xl font-bold border-2 border-[#d4dbe8] text-[#d4dbe8]"
            animate={{
      ...
      <div className="mt-8 text-center">
        {hasResult ? (
          ...
          <button
            onClick={rollDice}
            disabled={isRolling}
            className="px-8 py-3 bg-gradient-to-br from-[#d4dbe8] to-[#b41b60] text-white font-medium rounded-xl hover:bg-[#6d164f] transition-colors duration-200 disabled:opacity-50"
          >
