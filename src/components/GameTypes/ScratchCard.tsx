    ...
    <div className="flex flex-col items-center w-full">
      ...
      {!isModal && gameStarted && canScratch && !isRevealed && (
        <div className="w-full mb-4" style={{ maxWidth: `${width}px` }}>
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div 
              className="bg-gradient-to-br from-[#d4dbe8] to-[#b41b60] h-2 rounded-full transition-all duration-300" 
      ...
      <div 
        className={`relative rounded-xl overflow-hidden border-2 transition-all duration-200 shadow-lg bg-white ${
          isSelected 
            ? 'border-[#d4dbe8] shadow-xl ring-2 ring-[#d4dbe8]/20' 
            : selectable && !locked
              ? 'border-gray-200 hover:border-[#d4dbe8] cursor-pointer hover:shadow-xl' 
              : 'border-gray-200'
        } ${locked ? 'opacity-50' : ''}`}
        style={{
          width: `${width}px`,
