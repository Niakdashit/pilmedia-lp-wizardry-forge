...
const ResultScreenPreview: React.FC<ResultScreenPreviewProps> = ({
  ...
  // Vérifier si on doit afficher l'image de fond
  const displayType = currentConfig.displayType || 'message';
  const backgroundImage = currentConfig.backgroundImage;
  const shouldShowImage = displayType === 'image' && backgroundImage;

  return (
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0">
      {shouldShowImage ? (
        /* Affichage en mode image de fond */
        <div 
        ...
        >
          ...
          <button
            className="absolute left-1/2 transform -translate-x-1/2 px-6 py-2.5 font-medium text-sm transition-transform hover:-translate-y-[1px] cursor-default shadow-lg"
            style={{
              bottom: '20%',
          ...
          >
            {currentConfig.buttonText || 'Fermer'}
          </button>
        </div>
      ) : (
        /* Affichage en mode carte de message */
