...
useEffect(() => {
  ...
  const handleSaveCampaignName = useCallback(async () => {
    ...
    if (isSavingRef.current) {
      console.warn('⚠️ [JackpotEditor] Save already in progress, skipping duplicate call');
      return;
    }
    ...
    try {
      ...
      if (saved) {
        ...
        try { localStorage.setItem(`campaign:name:prompted:${(saved as any)?.id || currentId || 'new:jackpot'}`, '1'); } catch {}
  ...
  const handleBeforeOpenSettings = useCallback(async () => {
    ...
    if (campaignId && !campaignId.startsWith('temp-')) {
      ...
      try {
        ...
        console.error('❌ [JackpotEditor] Failed to save before opening settings:', e);
      }
    }
