...
const JackpotEditorLayout: React.FC<JackpotEditorLayoutProps> = ({ mode = 'campaign', hiddenTabs }) => {
  ...
  useEffect(() => {
    ...
    if (!campaignId || !isValidUuid(campaignId)) {
      if (!modeParam) {
        console.log('🎯 [JackpotEditor] No campaign ID and no mode → showing mode selection');
        setShowModeSelection(true);
      } else {
        console.log('🎯 [JackpotEditor] Mode already selected:', modeParam);
  ...
  const handleModeSelect = (selectedMode: 'fullscreen' | 'article') => {
    console.log('🎯 [JackpotEditor] Mode selected:', selectedMode);
    setShowModeSelection(false);
...
useEffect(() => {
  ...
  const handleSaveCampaignName = useCallback(async () => {
    ...
    const payload: any = isUuid(currentId) ? { id: currentId, name } : { name, type: 'jackpot' };
