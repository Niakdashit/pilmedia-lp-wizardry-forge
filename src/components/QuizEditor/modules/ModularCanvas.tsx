...
const ModularCanvas: React.FC<ModularCanvasProps> = ({ screen, modules, onUpdate, onDelete, onMove, onDuplicate, onSelect, selectedModuleId, device = 'desktop' }) => {
  ...
  // Séparer les modules Logo / Footer des autres modules
  ...
  const regularModules = React.useMemo(
    () => modules.filter((m) => m.type !== 'BlocLogo' && m.type !== 'BlocPiedDePage'),
    [modules]
  );
  ...
  return (
    <div className="w-full" data-modular-zone="1">
      <div className="flex flex-col" style={{ minHeight: minHeightPx }}>
        {/* TopSlot: Logo en pleine largeur */}
        {logoModules.map((m) => (
          <div
            key={m.id}
        ...
        {/* MiddleSlot: Modules réguliers dans le conteneur centré */}
