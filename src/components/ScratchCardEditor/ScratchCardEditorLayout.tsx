...
const handleSaveCampaignName = useCallback(async () => {
  ...
  if (saved) {
    ...
    try { localStorage.setItem(`campaign:name:prompted:${(saved as any)?.id || currentId || 'new:scratch'}`, '1'); } catch {}
  ...
  // Assurer la présence d'un bouton "Participer" sur l'écran 1 (au premier rendu)
  React.useEffect(() => {
  ...
  useEffect(() => {
    if (isNewCampaignGlobal) return; // ne pas auto-ajouter pendant new-campaign
    ...
    ensuredBlocBoutonRef.current = true;
  }, [isNewCampaignGlobal, modularPage.screens, currentScreen, persistModular, createDefaultBlocBouton, editorHasCardButton, campaignState?.id]);

  useEffect(() => {
    if (isNewCampaignGlobal) return; // ne pas auto-convertir/ajouter pendant new-campaign
  ...
  }, [isNewCampaignGlobal, canvasElements, modularPage.screens, currentScreen, persistModular, createDefaultBlocBouton, campaignState?.id]);
  ...
  const handleBackgroundChange = (bg: any, options?: { screenId?: 'screen1' | 'screen2' | 'screen3'; applyToAllScreens?: boolean; device?: 'desktop' | 'tablet' | 'mobile' }) => {
    ...
    if (options?.applyToAllScreens) {
      ...
      // Propager via évènement global pour que tous les canvas écoutent
