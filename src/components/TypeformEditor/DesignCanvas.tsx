...
export interface DesignCanvasProps {
  ...
  // Contenu personnalisé à rendre dans la safe-zone (frame navigateur)
  renderSafeZoneContent?: (params: { screenId: CanvasScreenId; selectedDevice: DeviceType; safeZonePadding: number; safeZoneRadius: number }) => React.ReactNode;
}
...
const DesignCanvas = React.forwardRef<HTMLDivElement, DesignCanvasProps>(({ 
  ...
  // Aligner le format mobile sur DesignEditor/FormEditor (ratio 9:16)
  const effectiveCanvasSize = useMemo(() => {
    if (selectedDevice === 'mobile') {
      // 360x640 offre une échelle identique et plus confortable
  ...
  const handleElementUpdate = useCallback((id: string, updates: any) => {
    ...
    if (workingUpdates.x !== undefined && workingUpdates.y !== undefined) {
      ...
      if (element) {
        ...
        // Disable clamping to allow full free move
        const clampedAbsX = absX;
        const clampedAbsY = absY;
  ...
  const handleRemoveBackground = useCallback(() => {
    if (background && background.type !== 'color') {
      // Remettre le background par défaut (blanc) et notifier le parent (source de vérité)
  ...
  // ============================================
  // RENDU PRINCIPAL - Mode fullscreen et article géré par DesignEditorLayout
  // ============================================
        ...
        >
          ...
          >
            ...
            >
              ...
              {/* Safe zone / custom content overlay */}
              <div className="absolute inset-0 z-[6]">
                ...
                {renderSafeZoneContent && (
                  ...
                  >
                    {renderSafeZoneContent({ screenId, selectedDevice, safeZonePadding, safeZoneRadius })}
                  </div>
                )}
              </div>
