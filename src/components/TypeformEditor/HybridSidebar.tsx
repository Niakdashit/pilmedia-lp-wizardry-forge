...
const HybridSidebar = forwardRef<HybridSidebarRef, HybridSidebarProps>(({
  ...
  // Overlay Prosplay AI (suggestions de questions)
  const [showProsplayAI, setShowProsplayAI] = useState(false);
  ...
  // Création d'une question Typeform depuis le sélecteur de layout vide (TypeformPreview)
  React.useEffect(() => {
    ...
    window.addEventListener('typeform-empty-layout-apply', handler as EventListener);
    return () => {
      window.removeEventListener('typeform-empty-layout-apply', handler as EventListener);
    };
  }, [setCampaign]);
  ...
  const renderPanel = (tabId: string) => {
    switch (tabId) {
      ...
      case 'questions':
        return (
          <QuestionsPanel
            questions={campaign?.typeformQuestions || []}
            onQuestionsChange={(questions) => {
              setCampaign({ ...campaign, typeformQuestions: questions });
            }}
  ...
  if (isPortraitFormat) {
    return (
      <MobileBottomSidebarLayout
        ...
        debugNamespace="ReferenceEditor"
      />
    );
  }
  ...
  return (
    <div data-hybrid-sidebar="expanded" className="flex h-full min-h-0 relative">
      ...
      {internalActiveTab && (
        <div className="bg-white border-r border-[hsl(var(--sidebar-border))] flex flex-col h-full min-h-0 shadow-sm" style={{ width: '22rem' }}>
          <div className="p-6 border-b border-[hsl(var(--sidebar-border))] bg-[hsl(var(--sidebar-surface))]">
            {(() => {
              // Dans le Typeform editor, afficher "Formulaire" au lieu de "Écran 1/2/3"
              const screenTitle = 'Formulaire';
              return (
      ...
      <ProsplayAIOverlay
        open={showProsplayAI}
        onClose={() => setShowProsplayAI(false)}
        existingQuestions={campaign?.typeformQuestions || []}
        onApply={(suggested) => {
          const existing = Array.isArray(campaign?.typeformQuestions) ? campaign?.typeformQuestions : [];
          const merged = [...existing, ...suggested];
          setCampaign({ ...campaign, typeformQuestions: merged });
          setShowProsplayAI(false);
        }}
      />
    </div>
  );
