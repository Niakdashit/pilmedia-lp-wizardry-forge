...
import TypeformPreview from './components/TypeformPreview';
...
const handleSaveCampaignName = useCallback(async () => {
  ...
  // Masquer le conteneur hérité TemplatedQuiz dans le contexte du TypeformEditor
  useEffect(() => {
    if (typeof window === 'undefined') return;

    const hideQuizPreviewContainer = () => {
      const el = document.getElementById('quiz-preview-container');
      if (el) {
    ...
    hideQuizPreviewContainer();

    // Observer le DOM au cas où le conteneur serait recréé
    const observer = new MutationObserver(() => hideQuizPreviewContainer());
  ...
  // Dans le Typeform editor, on force toujours screen2 (Formulaire unique)
  const [currentScreen, setCurrentScreen] = useState<'screen1' | 'screen2' | 'screen3'>('screen2');
  ...
  // DÉSACTIVÉ dans le Typeform editor : on reste toujours sur screen2
  useEffect(() => {
    // Dans le Typeform editor, pas de navigation multi-écrans
    return;
  }, []);
          ...
          <>
            ...
            >
              <div className="min-h-full flex flex-col">
                {/* Premier Canvas - MASQUÉ dans le Typeform editor */}
                {false && (
                ...
                {/* Deuxième Canvas - Écran unique "Formulaire" dans le Typeform editor */}
                ...
                <div className="mt-4 relative" data-screen-anchor="screen2">
                  ...
                  <div className="relative z-10">
                    {/* Canvas écran 2 classique pour conserver la même hauteur / géométrie, avec TypeformPreview dans la safe-zone */}
                    <DesignCanvas
                      editorMode={editorMode}
                      screenId="screen2"
                      selectedDevice={selectedDevice}
                      ...
                      quizModalConfig={quizModalConfig}
                      containerClassName={mode === 'template' ? 'bg-gray-50' : undefined}
                      elementFilter={(_element: any) => false}
                      renderSafeZoneContent={() => {
                        // Générer les teintes de couleurs
                        ...
                        const primaryColor = campaignState?.design?.typeformPrimaryColor || campaignState?.design?.primaryColor || '#841b60';
                        const fontFamily = campaignState?.design?.typeformFontFamily || 'Inter, sans-serif';
                ...
                {/* Troisième Canvas - MASQUÉ dans le Typeform editor */}
                {false && editorMode === 'fullscreen' && (
