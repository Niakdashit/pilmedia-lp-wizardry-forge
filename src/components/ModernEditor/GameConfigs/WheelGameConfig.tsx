...
const WheelGameConfig: React.FC<WheelGameConfigProps> = ({
  ...
  return <div className="space-y-6">
      ...
      <div className="space-y-4 p-4 bg-gray-50 rounded-lg">
        ...
        {campaign.gameConfig?.wheel?.mode === 'instant_winner' && <div className="space-y-2">
        ...
        })} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#841b60] focus:border-transparent" placeholder="Faire tourner" />
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex space-x-2">
            <button onClick={addSegmentPair} className="px-3 py-1 text-sm bg-[#841b60] text-white rounded-lg hover:bg-[#6d164f] transition-colors">
              + Ajouter une paire
            </button>
            {segments.length >= 2 && <button onClick={removeSegmentPair} className="px-3 py-1 text-sm bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors">
                - Retirer une paire
              </button>}
          </div>
        </div>
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto">
          ...
          return <div key={segment.id || index} className={`flex items-center space-x-2 p-3 border rounded-lg ${isLosingSegment ? 'border-red-200 bg-red-50' : 'border-green-200 bg-green-50'}`}>
                <div className={`w-3 h-3 rounded-full ${isLosingSegment ? 'bg-red-400' : 'bg-green-400'}`} />
                <input type="color" value={segment.color || '#841b60'} onChange={e => updateSegment(index, 'color', e.target.value)} className="w-8 h-8 rounded border border-gray-300" />
                <input type="text" value={segment.label || ''} onChange={e => updateSegment(index, 'label', e.target.value)} placeholder={isLosingSegment ? "Texte perdant" : "Texte gagnant"} className="flex-1 px-2 py-1 text-sm border border-gray-300 rounded focus:ring-1 focus:ring-[#841b60] focus:border-transparent" />
                <input type="color" value={segment.textColor || '#ffffff'} onChange={e => updateSegment(index, 'textColor', e.target.value)} title="Couleur du texte" className="w-8 h-8 rounded border border-gray-300" />
                {campaign.gameConfig?.wheel?.mode === 'probability' && <input type="number" min="1" value={segment.probability || 1} onChange={e => updateSegment(index, 'probability', parseInt(e.target.value))} className="w-16 px-2 py-1 text-sm border border-gray-300 rounded" title="Poids" />}
                <span className={`text-xs px-2 py-1 rounded-full ${isLosingSegment ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
        ...
        {segments.length > 0 && <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
