...
const ProEditorLayout: React.FC<ProEditorLayoutProps> = ({ mode = 'campaign', hiddenTabs }) => {
  ...
  console.log('🎨 [ProEditorLayout] Editor Mode:', editorMode);
...
const { syncAllStates } = useCampaignStateSync();
  ...
  useEditorUnmountSave('pro', {
    ...
    selectedDevice,
    canvasZoom,
    gameConfig: (campaignState as any)?.proConfig
  }, saveCampaign);
  ...
  useAutoSaveToSupabase(
    {
      campaign: {
        ...campaignState,
        type: 'pro',
        proConfig: (campaignState as any)?.proConfig
      },
      canvasElements,
      modularPage,
      screenBackgrounds,
    ...
    },
    {
      ...
      onSave: () => {
        console.log('✅ [ProEditor AutoSave] Campaign auto-saved to Supabase');
      },
      onError: (error) => {
        console.error('❌ [ProEditor AutoSave] Auto-save failed:', error);
      }
    }
  );
...
useEffect(() => {
  ...
  if (isTempCampaignId(campaignId)) {
    console.log('⏭️ [ProEditor] Skipping load for temporary campaign:', campaignId);
    return;
  }
  ...
  if (currentCampaignId && currentCampaignId !== campaignId) {
    console.log('🔄 [ProEditor] Switching campaigns');
  }
  ...
  if (currentCampaignId === campaignId) {
    ...
    if (hasPayload) {
      console.log('✅ [ProEditor] Campaign already loaded:', campaignId);
      return;
    }
    console.log('⚠️ [ProEditor] Campaign id matches but store empty, forcing reload');
  }
  
  console.log('🔄 [ProEditor] Loading campaign:', campaignId);
  ...
  const loadCampaignData = async () => {
    try {
      ...
      if (data) {
        console.log('✅ [ProEditor] Campaign loaded from DB:', {
          id: data.id,
          name: data.name,
      ...
      console.error('❌ [ProEditor] Failed to load campaign:', error);
    }
  };
...
useEffect(() => {
  ...
  if (!cid) {
    console.log('🆕 [ProEditor] Creating new blank campaign');
    beginNewCampaign('pro');
    // Générer un ID temporaire unique
    const tempId = generateTempCampaignId('pro');
    ...
    initializeNewCampaignWithId('pro', tempId);
...
useEffect(() => {
  ...
  if (didTempCleanupRef.current.has(id)) {
    console.log('⏭️ [ProEditor] Already cleaned temp campaign:', id);
    return;
  }
  
  console.log('🧹 [ProEditor] Cleaning temp campaign (first time):', id);
...
useEffect(() => {
  ...
  setCampaign((prev: any) => {
    ...
    const next = {
      ...prev,
      // N'écrase JAMAIS modularPage DB avec un vide local
...
useEffect(() => {
  ...
  const t = window.setTimeout(async () => {
    try {
      ...
      console.log('💾 [ProEditor] Autosave complet → DB', {
      ...
      console.warn('⚠️ [ProEditor] Autosave failed', e);
    }
  }, 1000);
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      try {
        // Guard: ne pas écrire si ce n'est pas la campagne sélectionnée
        const id = (campaignState as any)?.id as string | undefined;
        if (selectedCampaignId && id && id !== selectedCampaignId) return;
        setCampaign((prev: any) => {
          if (!prev) return prev;
  ...
  useEffect(() => {
    ...
    const promptedKey = id ? `campaign:name:prompted:${id}` : `campaign:name:prompted:new:pro`;
  ...
  const handleSaveCampaignName = useCallback(async () => {
    ...
    if (isSavingRef.current) {
      console.warn('⚠️ [ProEditor] Save already in progress, skipping duplicate call');
      return;
    }
    ...
    try {
      ...
      if (saved) {
        ...
        try { localStorage.setItem(`campaign:name:prompted:${(saved as any)?.id || currentId || 'new:pro'}`, '1'); } catch {}
  ...
  const handleBackgroundChange = (bg: any, options?: { screenId?: 'screen1' | 'screen2' | 'screen3'; applyToAllScreens?: boolean; device?: 'desktop' | 'tablet' | 'mobile' }) => {
    console.log('🎨 [ProEditor] handleBackgroundChange:', { bg, options });
  ...
  const currentJackpotConfig = useMemo(() => {
    return (campaignState as any)?.proConfig
      || (campaignConfig as any)?.gameConfig?.pro
      || {};
  ...
  const campaignData = useMemo(() => {
    ...
    const proConfig = (campaignState as any)?.proConfig || {
      reels: 3,
    };

    return {
      id: (campaignState as any)?.id || (campaignConfig as any)?.id || 'pro-design-preview',
      type: 'pro',
      design: {
      ...
      screens: [
        {
          ...
          description: descriptionElement?.content || 'Lancez le pro pour découvrir votre gain.',
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    const transformedCampaign = {
      ...
      name: 'Ma Campagne',
      type: (campaignData.type || 'wheel') as 'wheel' | 'scratch' | 'pro' | 'quiz' | 'dice' | 'form' | 'memory' | 'puzzle',
      design: {
        ...campaignData.design,
      ...
      setCampaign((prev: any) => {
        ...
        if (process.env.NODE_ENV !== 'production') {
          try {
            ...
            console.debug('🎯 [ProEditorLayout] Preserving wheel segments during merge', {
  ...
  const handleCTAClick = () => {
    console.log('🎯 [ProEditor] CTA clicked, moving to form step');
    setCurrentStep('form');
  };

  const handleFormSubmit = (data: Record<string, string>) => {
    console.log('📝 [ProEditor] Form submitted:', data);
    setCurrentStep('game');
  };

  const handleGameComplete = () => {
    console.log('🎮 [ProEditor] Game completed');
  ...
  const handleSaveAndQuit = useCallback(async () => {
    ...
    if (!isValidName) {
      console.log('❌ [ProEditor] Invalid campaign name, opening name modal before save');
      setIsNameModalOpen(true);
      return;
    }
  ...
  const handleBeforeOpenSettings = useCallback(async () => {
    console.log('🔄 [ProEditor] Preparing to open settings - syncing all states...');
    ...
    console.log('📊 [ProEditor] States to sync:', {
    ...
    if (campaignId && !campaignId.startsWith('temp-')) {
      console.log('💾 [ProEditor] Saving existing campaign before opening settings...');
      ...
      console.log('💾 [ProEditor] Payload to save:', {
        id: payload.id,
      ...
      try {
        const saved = await saveCampaignToDB(payload, saveCampaign);
        console.log('✅ [ProEditor] Campaign saved before opening settings:', saved?.id);
      } catch (e) {
        console.error('❌ [ProEditor] Failed to save before opening settings:', e);
      }
    }
  ...
  useEffect(() => {
    const handleSyncRequest = () => {
      console.log('🎯 [ProEditor] SYNC EVENT RECEIVED: campaign:sync:before-save');
      ...
      syncAllStates({
        ...
        selectedDevice,
        canvasZoom,
        proConfig: currentJackpotConfig,
        quizConfig,
        campaignConfig,
      ...
      setTimeout(() => {
        try {
          ...
          console.warn('⚠️ [ProEditor] Failed to dispatch campaign:sync:completed', error);
        }
      }, 50);
    };
  ...
  return (
    <Suspense fallback={
      <div className="app-viewport-container min-h-screen w-full flex items-center justify-center">
        <div className="text-white text-lg">Chargement...</div>
      </div>
    }>
      <div
        className="app-viewport-container min-h-screen w-full"
        style={{
                  ...
                  <DesignCanvas
                    ...
                    onShowDesignPanel={(context?: 'fill' | 'border' | 'text') => {
                     ...
                     onOpenElementsTab={() => {
                       ...
                       setActiveSidebarTab('pro');
                       // S'assurer que l'onglet pro est actif
                       if (sidebarRef.current) {
                         sidebarRef.current.setActiveTab('pro');
                       }
                       // Fermer les autres panneaux
                       setShowAnimationsInSidebar(false);
                       setShowPositionInSidebar(false);
                    ...
                    modularModules={(() => {
                      ...
                      console.log('📦 [ProEditor] Passing modularModules to screen1 DesignCanvas:', {
