...
const ScratchToolbar: React.FC<ScratchToolbarProps> = React.memo(({
  ...
  const handleOpenSettings = useCallback(async () => {
    try {
      ...
      const payload: any = {
        ...(campaignState || {}),
        name: (campaignState as any)?.name || 'Nouvelle campagne pro',
        type: 'pro',
        ...
        {showSaveCloseButtons && (
          <>
            ...
            <button 
              ...
              className="flex items-center px-3 py-1.5 text-xs sm:text-sm rounded-lg transition-colors bg-[#44444d] text-white hover:opacity-95"
