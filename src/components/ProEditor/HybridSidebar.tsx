...
const HybridSidebar = forwardRef<HybridSidebarRef, HybridSidebarProps>(({
  ...
  // La logique de changement d'onglet est gérée par ProEditorLayout
  ...
  React.useEffect(() => {
    ...
    // Si un autre panneau a été activé, basculer vers son onglet correspondant
      ...
      const activatedPanel = panelStates.find(p => p.active && !p.prevActive && p.key !== 'background' && p.key !== 'quiz' && p.key !== 'wheel' && p.key !== 'pro');
      if (activatedPanel) {
        newActiveTab = activatedPanel.key;
        shouldUpdate = true;
      } 
      // Si l'onglet actif est un panneau qui a été désactivé, revenir à 'game' pour wheel/pro ou 'elements' pour les autres
      else if (panelStates.some(p => p.key === internalActiveTab && !p.active && p.prevActive)) {
        newActiveTab = (internalActiveTab === 'wheel' || internalActiveTab === 'pro') ? 'game' : 'elements';
        shouldUpdate = true;
      }
    }
