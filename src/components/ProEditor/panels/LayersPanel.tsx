...
const LayersPanel: React.FC<LayersPanelProps> = ({ 
  ...
  // Context menu state for layer ordering actions
...
const CustomDragPreview: React.FC = () => {
  // Hide preview to avoid the persistent artifact
  return null;
};
  ...
  // Helpers for one-step and extreme moves (context menu actions)
  const moveToFront = useCallback((item: DragItem) => {
    const units = buildTopUnits();
    if (!item.parentId) {
      // Top-level element or group
      const idx = units.findIndex((u) => u.id === item.id && u.kind === item.kind);
    ...
    // Inside a group
    const groupUnit = units.find((u) => u.kind === 'group' && u.id === item.parentId);
    if (!groupUnit || !groupUnit.children) return;
    const children = [...groupUnit.children];
    const cIdx = children.findIndex((c) => c.id === item.id);
    if (cIdx <= 0) return;
    const [movedChild] = children.splice(cIdx, 1);
    children.unshift(movedChild);
    groupUnit.children = children;
    assignZIndicesFromUnits(units);
  }, [assignZIndicesFromUnits, buildTopUnits]);

  const sendToBack = useCallback((item: DragItem) => {
    const units = buildTopUnits();
    if (!item.parentId) {
      const idx = units.findIndex((u) => u.id === item.id && u.kind === item.kind);
      if (idx === -1 || idx === units.length - 1) return; // not found or already at back
      const [moved] = units.splice(idx, 1);
      units.push(moved);
      assignZIndicesFromUnits(units);
      return;
    ...
    const children = [...groupUnit.children];
    const cIdx = children.findIndex((c) => c.id === item.id);
    if (cIdx === -1 || cIdx === children.length - 1) return;
    const [movedChild] = children.splice(cIdx, 1);
    children.push(movedChild);
    groupUnit.children = children;
    assignZIndicesFromUnits(units);
  }, [assignZIndicesFromUnits, buildTopUnits]);

  const bringForward = useCallback((item: DragItem) => {
    const units = buildTopUnits();
    if (!item.parentId) {
      const idx = units.findIndex((u) => u.id === item.id && u.kind === item.kind);
      if (idx <= 0) return;
      const [moved] = units.splice(idx, 1);
      units.splice(idx - 1, 0, moved);
      assignZIndicesFromUnits(units);
      return;
    }
    const groupUnit = units.find((u) => u.kind === 'group' && u.id === item.parentId);
    ...
    if (cIdx <= 0) return;
    const [movedChild] = children.splice(cIdx, 1);
    children.splice(cIdx - 1, 0, movedChild);
    groupUnit.children = children;
    assignZIndicesFromUnits(units);
  }, [assignZIndicesFromUnits, buildTopUnits]);

  const sendBackward = useCallback((item: DragItem) => {
    const units = buildTopUnits();
    if (!item.parentId) {
      const idx = units.findIndex((u) => u.id === item.id && u.kind === item.kind);
  ...
  const LayerRow: React.FC<{ layer: LayerItem; depth: number }> = ({ layer, depth }) => {
    ...
    const [{ isDragging }, dragRef] = useDrag(() => ({
      ...
      canDrag: () => {
        console.log('🎯 canDrag check for element:', layer.id, 'isLocked:', isLocked, 'canDrag:', !isLocked);
        return !isLocked;
      },
  ...
  return (
    <div className="p-4">
      ...
      {contextMenu?.visible && (
        <div
          className="fixed z-50 bg-white border border-gray-200 rounded-md shadow-lg py-0.5"
          style={{ left: contextMenu.x, top: contextMenu.y, width: 'max-content' }}
          onMouseDown={(e) => e.stopPropagation()}
        >
          <button
            className="w-full text-left px-2.5 py-1 text-[11px] hover:bg-gray-100 whitespace-nowrap"
            onClick={() => {
              console.log('🔝 LayersPanel moveToFront called for:', contextMenu.target);
              moveToFront(contextMenu.target);
              setContextMenu(null);
            }}
          >
          ...
          <button
            className="w-full text-left px-2.5 py-1 text-[11px] hover:bg-gray-100 whitespace-nowrap"
            onClick={() => {
              console.log('🔻 LayersPanel sendToBack called for:', contextMenu.target);
              sendToBack(contextMenu.target);
              setContextMenu(null);
            }}
          >
