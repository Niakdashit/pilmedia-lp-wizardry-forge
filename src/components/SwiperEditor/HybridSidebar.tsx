...
import ButtonModulePanel from '../SwiperEditor/modules/ButtonModulePanel';
import VideoModulePanel from '../SwiperEditor/modules/VideoModulePanel';
...
interface HybridSidebarProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  // Inline swiper panel controls
  showSwiperPanel?: boolean;
  onSwiperPanelChange?: (show: boolean) => void;
  ...
  // Swiper config values & callbacks (provided by parent)
  swiperQuestionCount?: number;
  swiperTimeLimit?: number;
  swiperDifficulty?: 'easy' | 'medium' | 'hard';
  swiperBorderRadius?: number;
  swiperWidth?: string;
  swiperMobileWidth?: string;
  selectedSwiperTemplate?: string;
  ...
  onSwiperWidthChange?: (width: string) => void;
  onSwiperMobileWidthChange?: (width: string) => void;
  ...
  onSwiperQuestionCountChange?: (count: number) => void;
  onSwiperTimeLimitChange?: (time: number) => void;
  onSwiperDifficultyChange?: (difficulty: 'easy' | 'medium' | 'hard') => void;
  onSwiperBorderRadiusChange?: (radius: number) => void;
  onSwiperTemplateChange?: (templateId: string) => void;
...
const HybridSidebar = forwardRef<HybridSidebarRef, HybridSidebarProps>(({
  ...
  swiperBorderRadius,
  onSwiperBorderRadiusChange,
  showSwiperPanel = false,
  onSwiperPanelChange,
  ...
  onAddToHistory,
  swiperQuestionCount = 5,
  swiperTimeLimit = 30,
  swiperDifficulty = 'medium',
  selectedSwiperTemplate,
  onSwiperQuestionCountChange,
  onSwiperTimeLimitChange,
  onSwiperDifficultyChange,
  onSwiperTemplateChange: handleSwiperTemplateChange,
  onSwiperWidthChange: handleSwiperWidthChange,
  onSwiperMobileWidthChange: handleSwiperMobileWidthChange,
  ...
  const activeTemplate = React.useMemo(() => {
    return swiperTemplates.find((tpl) => tpl.id === selectedSwiperTemplate) || swiperTemplates[0];
  }, [selectedSwiperTemplate]);
  ...
  React.useEffect(() => {
    ...
    const panelStates = [
      ...
      { key: 'swiper', active: showSwiperPanel, prevActive: prev.showSwiperPanel },
    ...
    // Si le panneau Swiper est activé, forcer l'onglet swiper
    if (showSwiperPanel && !prev.showSwiperPanel) {
      newActiveTab = 'swiper';
      shouldUpdate = true;
    }
    ...
    // Si un autre panneau a été activé, basculer vers son onglet correspondant
      ...
      const activatedPanel = panelStates.find(p => p.active && !p.prevActive && p.key !== 'background' && p.key !== 'swiper' && p.key !== 'wheel');
      if (activatedPanel) {
  ...
  React.useEffect(() => {
    ...
    const isTransientSwiper = internalActiveTab === 'swiper' && showSwiperPanel;
    const isTransientWheel = internalActiveTab === 'wheel' && showWheelPanel;

    if (!activeIsVisible && !isTransientSwiper && !isTransientWheel) {
      setInternalActiveTab(backgroundVisible ? 'background' : (tabs[0]?.id ?? null));
    }
  }, [tabs, internalActiveTab, showSwiperPanel, showWheelPanel]);
...
// Export with explicit name for better debugging
