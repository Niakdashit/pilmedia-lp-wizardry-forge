    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-100">
      <div className="p-4 sm:p-6 border-b border-gray-200">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
          ...
          <Link
            to="/admin/campaigns"
            className="inline-flex items-center px-4 py-2 bg-gradient-to-br from-[#d4dbe8] to-[#b41b60] text-white text-sm font-medium rounded-lg hover:bg-[#6d164f] transition-colors"
          >
            Voir tout
          </Link>
        </div>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {campaigns.map((campaign) => (
              <tr key={campaign.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4">
                  <Link to={`/admin/clients/${campaign.clientId}`} className="text-sm text-[#d4dbe8] hover:underline">
                    {campaign.clientName}
                  </Link>
                </td>
                <td className="px-6 py-4">
        ...
        {campaigns.map((campaign) => (
          <div key={campaign.id} className="p-4 sm:p-6">
            ...
            <div className="flex items-center justify-between text-xs text-gray-500 mb-3">
              <Link to={`/admin/clients/${campaign.clientId}`} className="text-[#d4dbe8] hover:underline">
                {campaign.clientName}
              </Link>
