...
import { SOCIAL_PRESETS, getSocialIconUrl } from '@/components/ScratchCardEditor/modules/socialIcons';
import type { DesignScreenId, DesignModule, DesignModuleType } from '@/types/designEditorModular';

export interface DesignModulesPanelProps {
  currentScreen: DesignScreenId;
  onAdd: (screen: DesignScreenId, module: DesignModule) => void;
}

const createModule = (type: DesignModuleType, screen: DesignScreenId): DesignModule => {
  ...
  switch (type) {
    ...
    case 'BlocSeparateur':
      return { id, type, spacingTop: 8, spacingBottom: 8, minHeight: 80, layoutWidth: 'full' } as any;
    case 'BlocVideo':
      return {
        id,
        type,
...
const items: Array<{ id: DesignModuleType; label: string; icon: React.ComponentType<any> }> = [
...
const DesignModulesPanel: React.FC<DesignModulesPanelProps> = ({ currentScreen, onAdd }) => {
  return (
...
export default DesignModulesPanel;
