...
export interface DesignCanvasProps {
  ...
  // Open the temporary wheel configuration panel
...
const DesignCanvas = React.forwardRef<HTMLDivElement, DesignCanvasProps>(({ 
  ...
  // Protection: s'assurer que elements est toujours un tableau
  ...
  // When a module is selected inside ModularCanvas, ignore canvas click/marquee briefly to avoid flicker
  const lastModuleSelectAtRef = useRef<number>(0);

  useEffect(() => {
    const onModuleSelected = () => {
      lastModuleSelectAtRef.current = Date.now();
      // Strongly suppress any immediate canvas click/marquee that could clear selection
      suppressNextClickClearRef.current = true;
      window.setTimeout(() => {
        suppressNextClickClearRef.current = false;
      }, 300);
    };
    window.addEventListener('modularModuleSelected', onModuleSelected as any);
    return () => window.removeEventListener('modularModuleSelected', onModuleSelected as any);
  }, []);
  ...
  const alignmentElements = useMemo(() => {
    if (!safeElements || !Array.isArray(safeElements)) return [];
    return safeElements.map((el: any) => {
  ...
  const computeContentBounds = useCallback((): { x: number; y: number; width: number; height: number } | null => {
    ...
    const source = (selectedElements && selectedElements.length > 0) ? selectedElements : safeElements;
  ...
  }, [selectedElements, safeElements, measuredBounds]);
  ...
  const elementById = useMemo(() => {
    ...
    for (const el of safeElements) m.set(el.id, el);
    return m;
  }, [safeElements]);

  const devicePropsById = useMemo(() => {
    const m = new Map<string, any>();
    for (const el of safeElements) m.set(el.id, getPropertiesForDevice(el, selectedDevice));
    return m;
  }, [safeElements, selectedDevice, getPropertiesForDevice]);
  ...
  const handleCanvasPaste = useCallback(() => {
    if (clipboard && clipboard.type === 'element') {
      ...
      const updatedElements = [...safeElements, newElement];
      onElementsChange(updatedElements);
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <MobileResponsiveLayout
        ...
        className={`design-canvas-container flex-1 flex flex-col items-center ${isWindowMobile ? 'justify-center py-2' : 'justify-start pt-40 pb-4'} px-4 ${containerClassName ? containerClassName : 'bg-gray-100'} relative`}
        // Props pour la sidebar mobile
        ...
        <div 
          ...
          >
            <div 
              ...
              onClick={(e) => {
                // Fallback stabilization: disable click-to-clear to prevent race with module selection
                return;
              }}
              onPointerDown={(e) => {
                // Start marquee only when pressing on empty canvas (not on modular content)
                if (readOnly) return;
                // Ignore shortly after a module selection to prevent starting marquee
                if (Date.now() - lastModuleSelectAtRef.current < 200) return;
                const target = e.target as HTMLElement | null;
                if (target && (target.closest('[data-modular-zone="1"]') || target.closest('[data-module-id]'))) {
                  return; // let ModularCanvas handle it
                }
                handleBackgroundPointerDown(e as unknown as React.PointerEvent<HTMLDivElement>);
              }}
            >
            ...
            >
              ...
              {!readOnly && (
                <div className="absolute bottom-2 right-2" style={{ zIndex: 5000, pointerEvents: 'auto' }}>
                  <WheelSettingsButton
                    onClick={() => {
                ...
                <>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      // Put ModularCanvas below design elements
                      zIndex: 0,
                      // If no regular modules, do not intercept clicks
                      pointerEvents: regularModules.length > 0 ? 'auto' : 'none'
                    }}
                    data-modular-layer="regular"
                  >
                    ...
                    <div className="w-full max-w-[1500px] flex" style={{ minHeight: effectiveCanvasSize?.height || 640 }}>
                      <ModularCanvas
                        ...
                        device={selectedDevice}
                        selectedModuleId={selectedModuleId as any}
                        ...
                        onSelect={(m) => {
                          // Immediately suppress canvas clear/marquee to avoid flicker
                          suppressNextClickClearRef.current = true;
                          window.setTimeout(() => { suppressNextClickClearRef.current = false; }, 600);
                          try {
            ...
            {elementsSortedByZIndex
              ...
              return (
                <CanvasElement 
                  ...
                  onOpenWheelPanel={
                    (elementForCanvas as any)?.type === 'wheel'
                      ? () => {
                          try { onWheelPanelChange?.(true); } catch {}
                          try { onOpenWheelPanel?.(); } catch {}
                        }
                      : undefined
                  }
                />
