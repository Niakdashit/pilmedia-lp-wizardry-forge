...
const TextPanel: React.FC<TextPanelProps> = ({
  ...
  return <div className="p-4 space-y-6">
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          className={`p-2 text-sm rounded transition-all ${activeTab === 'combinaisons' ? 'bg-[#44444d] text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'}`}
          onClick={() => setActiveTab('combinaisons')}
        >
          Combinaisons
        </button>
        <button
          className={`p-2 text-sm rounded transition-all ${activeTab === 'polices' ? 'bg-[#44444d] text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'}`}
      ...
      <div className="space-y-6">
          ...
          <div>
            <button onClick={() => addText()} className="w-full p-4 border-2 border-dashed border-gray-300 rounded-lg hover:border-[hsl(var(--primary))] hover:bg-[#44444d] hover:text-white transition-colors flex items-center justify-center group">
              ...
              <div className="grid grid-cols-1 gap-3 max-h-72 overflow-y-auto">
                {titlePresets.map((p) => (
                  <button
                    ...
                    className="p-3 border border-gray-200 rounded hover:border-[hsl(var(--primary))] hover:bg-[#44444d] hover:text-white transition-colors text-left group"
                  >
                    <div
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {fontCategories.map((category, index) => (
                    <button
                      key={index}
                      className={`p-2 text-xs rounded cursor-pointer transition-all duration-200 ${selectedCategory.name === category.name ? 'bg-[#44444d] text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'}`}
                ...
                <div className="grid grid-cols-1 gap-2 max-h-64 overflow-y-auto">
                  {selectedCategory.fonts.map(font => (
                    <button
                      ...
                      className="p-2 border border-gray-200 rounded hover:border-[hsl(var(--primary))] hover:bg-[#44444d] hover:text-white transition-colors text-left group"
                    >
