  ...
  const handleImageUpload = (file: File) => {
    ...
    reader.onload = e => {
      ...
      const newImage = {
        ...
        src: imageUrl,
        // Changé de 'url' à 'src' pour correspondre à EditableImage
        x: 50,
        y: 50,
        width: 150,
        height: 150,
  ...
  return <div className="space-y-6">
      ...
      <div className="premium-card mx-[30px]">
        ...
        <div className="space-y-4">
          ...
          {config.design?.customImages && config.design.customImages.length > 0 ? <div className="space-y-3">
              <h5 className="text-sidebar-text-primary text-sm font-medium">Images ajoutées :</h5>
              <div className="grid grid-cols-2 gap-3">
                {config.design.customImages.map((image: any) => <div key={image.id} className="relative group">
                    ...
                    <button onClick={() => removeImage(image.id)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-red-600 opacity-0 group-hover:opacity-100 transition-opacity">
                      ×
                    </button>
                  </div>)}
              </div>
