...
const WheelMechanicConfig: React.FC<WheelMechanicConfigProps> = ({ config, onConfigUpdate }) => {
  const wheelSegments: WheelSegment[] = config.wheelSegments || [
    { id: '1', label: 'Segment 1', color: '#FF6B6B', probability: 20, isWinning: true },
    { id: '2', label: 'Segment 2', color: '#4ECDC4', probability: 20, isWinning: false },
    { id: '3', label: 'Segment 3', color: '#45B7D1', probability: 20, isWinning: false },
    { id: '4', label: 'Segment 4', color: '#96CEB4', probability: 20, isWinning: true },
    { id: '5', label: 'Segment 5', color: '#FFEAA7', probability: 20, isWinning: false },
  ];

  const updateSegments = (newSegments: WheelSegment[]) => {
    onConfigUpdate({ wheelSegments: newSegments });
  };
  ...
  const updateSegment = (id: string, updates: Partial<WheelSegment>) => {
    updateSegments(wheelSegments.map(s => s.id === id ? { ...s, ...updates } : s));
  };
      ...
      <div className="premium-card">
        ...
        <div className="space-y-3">
          {wheelSegments.map((segment, index) => (
            <div key={segment.id} className="p-3 bg-sidebar-surface rounded-lg border border-sidebar-border">
