...
const GameRenderer: React.FC<GameRendererProps> = ({ gameType, config, device, onResult }) => {
  ...
  const gamePosition = config.deviceConfig?.[device]?.gamePosition || { x: 0, y: 0, scale: 1.0 };
  ...
  const getGameContainerStyle = (): React.CSSProperties => {
    return {
      ...
      width: '100%',
      height: '100%',
      minHeight: '400px', // Hauteur minimum pour assurer une bonne visibilité
      ...
      case 'scratch':
        return (
          <ScratchPreview
            ...
            onStart={() => console.log('Scratch started')}
            disabled={false}
            buttonLabel="Gratter"
