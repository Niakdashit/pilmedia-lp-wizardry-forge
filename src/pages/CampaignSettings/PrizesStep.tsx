...
const PrizesStep: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h2 className="font-semibold mb-2">Nombre de gagnants</h2>
        <input
          type="number"
          className="w-full px-3 py-2 border border-primary-200 rounded-lg max-w-xs focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand"
          value={(form as any)?.winners?.count ?? ''}
          onChange={e => handleChange('winners.count', e.target.value ? Number(e.target.value) : undefined)}
        />
      </div>
